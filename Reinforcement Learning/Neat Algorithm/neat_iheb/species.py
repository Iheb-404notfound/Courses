...
class Species:
    ...
    def breed(self):
        if np.random.uniform(0,1) < self.config.species_procreation:
    ...
    def natural_selection(self, fittest=-1):
        ...
        if fittest > 0 and fittest < len(byfitness):
            ...
            selected = byfitness[:int(np.ceil(self.config.species_procreation * len(byfitness)))]
    ...
    def update_fitness(self):
        ...
        if len(self.fitness_history) > self.config.species_max_age:
    ...
    def stagnated(self):
        ...
        return avg < self.fitness_history[0] and len(self.fitness_history) >= self.config.species_max_age
